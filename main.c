...
int main() {
    ...
    // Vetor com os 8 números inteiros escolhidos para adicionar como chaves na lista
    int numeros[] = {80, 20, 69, 10, 51, 19, 91, 15, 37, 26, 73};
    int i;
    ...
    for (i = 0; i < 11; i++) {
        REGISTRO reg;
        reg.chave = numeros[i];
        if (inserirElemLista(&lista, reg, i)) {
            printf("Inserido: %d\n", numeros[i]);
        } else {
